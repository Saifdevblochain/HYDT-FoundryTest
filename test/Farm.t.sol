...
contract HYDTTest is Test {
    ...
    function test_Initialize_RevertOn_HYGT_ZeroAddress() public {
        address[3] memory tokens;
        tokens = [address(wbnb), address(usdt), address(hygt)];
        uint256 initialMintStartTime_ = block.timestamp;
    ...
    function test_addPool_RevertOn_NonCallerRole() public {
        test_initialize();
        vm.roll(200);
        vm.prank(address(12345));
        vm.expectRevert(
            "AccessControl: account 0x0000000000000000000000000000000000003039 is missing role 0xd0990c50b6714f222e6fd1faaf5345bf1aa2867d2861fc2cc43b364e7d948647"
        );
        farm.addPool(IERC20(address(usdt)), uint(10), "testToken", true);
    }
    ...
    function test_Deposit() public {
        ...
        (uint amount0, ) = farm.userInfo(uint(1), address(this));
    ...
    function test_Deposit_MultipleDesposits() public {
        test_initialize();
        usdt.approve(address(farm), 10000e18);
        (uint amount0, ) = farm.userInfo(uint(1), address(this));
        usdt.approve(address(farm), 1000e18);
    ...
    function test_updateAllocPointOnly() public {
        ...
        (, uint256 allocPoint1, , ) = farm.poolInfo(uint(1));
        assertEq(allocPoint1, 50);
        uint res = farm.totalAllocPoint();
        assertEq(res, 650);
    }

    function test_updateAllocPoint_RevertOn_MissingRole() public {
        test_initialize();
        vm.roll(1011);
        vm.prank(address(1234));
        vm.expectRevert(
            "AccessControl: account 0x00000000000000000000000000000000000004d2 is missing role 0xd0990c50b6714f222e6fd1faaf5345bf1aa2867d2861fc2cc43b364e7d948647"
        );
        farm.updateAllocation(uint(1), uint(50), true);
    }
    ...
    function test_withdraw() public {
        ...
        (uint amount1, ) = farm.userInfo(uint(1), address(this));
    ...
    function test_EmergencyWithdraw() public {
        ...
        usdt.approve(address(farm), 1000e18);
        farm.deposit(uint(1), 100e18);
        (uint amount, ) = farm.userInfo(uint(1), address(this));
